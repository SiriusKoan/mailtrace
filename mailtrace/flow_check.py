...
class FlowStatus(Enum):
    """Status of a mail flow through the cluster."""
...
@dataclass
class MailFlow:
    """Represents a single mail flow from inbound to terminal state."""
...
class FlowCheckResult:
    """Aggregated result of a cluster flow conservation check."""
    ...
    def to_dict(self) -> dict:
        """Convert to dict with nested flows serialized."""
        return {
...
def classify_terminal_state(
    ...
    for entry in log_entries:
        ...
        if entry.service in _LOCAL_DELIVERY_SERVICES and _DELIVERED_RE.search(
            entry.message
        ):
            return (FlowStatus.COMPLETE, "delivered_locally")

        if entry.service in _RELAY_SERVICES and _DELIVERED_RE.search(
            entry.message
        ):
...
def _classify_flow_from_batch(
    ...
    has_bounce = False
    for entries in logs_by_qid.values():
...
def _trace_to_terminal(
    ...
    for entry in logs:
        if (
            is_out_of_time_window(entry.datetime, time, time_range)
            and mail_id not in out_of_window
        ):
